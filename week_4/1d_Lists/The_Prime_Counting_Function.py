...
# This function returns a list of prime numbers up to n (inclusive), using the Sieve of Eratosthenes
...
# Here we will use the sieve to compute the prime counting function.
# The sieve will return a list of all the primes up to n, so the prime counting function merely returns the length of this list!
...
# piUsingisPrime(n)
# Here we will use the isPrime function to compute the prime counting function
...
# before running the timing code below, let's run some test code to verify that the functions above seemingly work.
# Test values are from: http://en.wikipedia.org/wiki/Prime-counting_function
...
# timing code
# Finally we can time each version for a large value of n and compare their runtimes
import time
